...
def read_positions(pos_args: List[str], pos_file: Optional[str], vcf_file: Optional[str]) -> List[Tuple[str, int]]:
    """
    ...
    # From --vcf-file
    if vcf_file:
        positions.extend(read_vcf_positions(vcf_file))
...
def main():
    ...
    # Resolve positions from all sources
    positions = read_positions(args.pos, args.pos_file, args.vcf_file)
