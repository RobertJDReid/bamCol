...
def get_hard_clip_offset(aln: pysam.AlignedSegment) -> int:
    """
    Return the number of hard-clipped bases at the start of the read.
    This can be added to query_position to get the position in the
    original full-length read sequence.
    """
    if aln.cigartuples is None:
        return 0
    first_op, first_len = aln.cigartuples[0]
    # pysam CIGAR op code 5 = H (hard clip)
    if first_op == 5:
        return first_len
    return 0
...
def pileup_one_position(
    ...
    for col in cols:
        ...
        for pr in col.pileups:
            ...
            raw_qpos = pr.query_position # for indexing into query_sequence / query_qualities
            qpos = raw_qpos              # for reporting in read_pos output

            if adjust_hardclip and qpos is not None:
                qpos = qpos + get_hard_clip_offset(aln)

            if not is_del and not is_refskip and raw_qpos is not None:
                seq = aln.query_sequence
                quals = aln.query_qualities
                if seq is not None and 0 <= raw_qpos < len(seq):
                    base = seq[raw_qpos]
                if quals is not None and 0 <= raw_qpos < len(quals):
                    base_qual = int(quals[raw_qpos])
...
def worker_pileup(args) -> List[dict]:
    ...
    try:
        for rec in pileup_one_position(
            ...
            adjust_hardclip=adjust_hardclip,
...
def main():
    ...
    ap.add_argument(
        "--adjust-hardclip",
        action="store_true",
        help="Adjust query position output to account for hard clips",
    )
    ap.add_argument(
            ...
            try:
                ...
                for chrom, pos1 in pos_iter:
                    ...
                    for rec in pileup_one_position(
                        ...
                        adjust_hardclip=args.adjust_hardclip,
